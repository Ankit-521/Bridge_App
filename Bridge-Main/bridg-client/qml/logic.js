...
function openCards(player, the_cards) {
    ...
    for (var i in the_cards) {
        cards(player).model.append({"card": the_cards[i]});
    }
}
...
function showMove(player, type, data) {
    ...
    if (type === 0) {
        ...
        if (data==="pass") {
            bidPassCount++;
            displayBid = bidPassCount;
            if(bidPassCount===4 && lastSemiValidBid==="")
            {
                bidFlag="fourpasses";
                fourpasses="0";
                //fourpasses=parseInt(lastValidBid.split(":")[1].split(",")[1]);
                phase = 3;
                scoreTimer.start();
            }
            else if (bidPassCount===3 && lastSemiValidBid!="") {
                bidFlag="trumpSuit";
                stopBidTimer.start();
...
//ankit claim
function endbyclaim(num_tricks,isPlayer){
    var ourtricks;
    var totalcards=cards(me.num).count;
    if(isPlayer)ourtricks=num_tricks;
    else ourtricks=totalcards-num_tricks;
    scoreBoard.tricks+=ourtricks;
    scoreBoard.opponentTricks+=(totalcards-ourtricks);
//scoreTile.update needs to be done num_tricks times lower cards and opponentTricks times upper cards
    for(var k=0 ;k<4;k++){
    cards(k).model.clear();
    }


        for(var i=0;i<ourtricks;i++){

             scoreBoard.updateScoreTile(true);
        }
        for(var j=0;j<(totalcards-ourtricks);j++){
             scoreBoard.updateScoreTile(false);
        }
    phase=3;
    scoreTimer.start();
}
...
function addMessageChat(type, message, sender) {
    if(type === "global") {
        chat.addMessageGlobal(message, sender)
    } else if(type === "table") {
        chat.addMessageTable(message, sender)
    } else if(type === "partner") {
        chat.addMessagePartner(message, sender)
    }
}
