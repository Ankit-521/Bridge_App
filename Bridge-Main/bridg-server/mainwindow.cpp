...
MainWindow::MainWindow(QWidget *parent)
  ...
  m_video_chat_Manager = new MainVideoChatWindow();

  connect(ui->actionStop_Event, SIGNAL(triggered()), this, SLOT(stopEvent()));
  connect(ui->actionChat, &QAction::triggered, m_chat, &ChatForm::show);
}
...
void MainWindow::on_actionNew_Event_triggered() {
  if (m_server) {
    ...
    QMessageBox::StandardButton which = QMessageBox::question(
        this, "Event going on", message, QMessageBox::Yes | QMessageBox::No,
        QMessageBox::No);
    if (which == QMessageBox::Yes) {
      stopEvent();
      m_video_chat_Manager->eventStopped();
    } else
      return;
  }
  NewEventDialog dlg(this);
  if (dlg.exec()) {
    initialiseView(dlg.pairs(), dlg.boardCount(), dlg.movement(),
                   dlg.scoring());
...
void MainWindow::initialiseView(int pairs, int boardCount, int movement,
                                int scoring) {
  stopMovementModel();
  ...
  m_chat->addTables(m_model->tables());

  connect(ui->actionCompute_Result, SIGNAL(triggered()), m_model,
          SLOT(computeResult()));
  connect(m_model, SIGNAL(resultReady()), this, SLOT(showResult()));

  // Code added by wasswa Goffrey
  m_video_chat_Manager->setTables(m_model->tables());
}
...
void MainWindow::on_actionStart_Server_triggered() {
  ...
  connect(m_server, SIGNAL(clientConnected(int, int, bool)), this,
          SLOT(updateLights(int, int, bool)));
  connect(m_server, SIGNAL(scoreReceived(int, int)), m_model,
          SLOT(updateScore(int, int)));
  connect(m_server, &Server::incomingMessageFromClient, m_chat,
          &ChatForm::incomingMessageFromClient);
  connect(m_model, SIGNAL(tableChanged(int, int, int, int, int, int, int)),
          m_server, SLOT(notifyTableChange(int, int, int, int, int, int, int)));
  // connect(m_model, SIGNAL(tournamentEnded(int)), ui->actionCompute_Result,
  // SIGNAL(triggered()));
  connect(m_model, SIGNAL(tournamentEnded(int)), m_server,
          SLOT(notifyTournamentEnd(int)));
  connect(m_chat, &ChatForm::sendMessage, m_server,
          &Server::sendMessageFromServer);
  stopLogger();
  m_logger = new Logger(this);
  m_logger->initiate(m_server, m_model);

  // Code added By Wasswa Goeffrey
  // set up video connections

  connect(m_server, &Server::clientConnected, m_video_chat_Manager,
          &MainVideoChatWindow::playerJoined);
  connect(m_video_chat_Manager, &MainVideoChatWindow::send, m_server,
          &Server::sendVideoMessage);
}
...
void MainWindow::stopMovementModel() {
  if (m_model) {
    ...
    if (m_server)
      disconnect(m_server, 0, m_model, 0);
...
void MainWindow::updateLights(int table, int player, bool on) {
  ...
  if (!item)
    return;
  LightsWidget *w =
      qobject_cast<LightsWidget *>(ui->statusList->itemWidget(item, 1));
...
void MainWindow::on_actionQuit_triggered() {
  if (m_server) {
    ...
    if (QMessageBox::Yes ==
        QMessageBox::question(this, "Event going on", message,
...
void MainWindow::on_actionExport_Tournament_triggered() {
  if (!m_logger) {
    QMessageBox::warning(this, "No tournament",
                         "There is no tournament to export!");
    return;
  }
  QString defaultPath =
      QString("%1/%2.xml")
          .arg(QDir::homePath(), QDate::currentDate().toString());
  QString saveFilepath = QFileDialog::getSaveFileName(
      this, "Browse to the location for saving tournament file", defaultPath,
      "XML document (*.xml)");
...
void MainWindow::on_actionImport_Tournament_triggered() {
  QString logFile =
      QFileDialog::getOpenFileName(this, "Browse to the tournament file",
                                   QDir::homePath(), "XML document (*.xml)");
  if (logFile.isEmpty())
    return;
  loadLogFile(logFile);
}
...
void MainWindow::on_actionExport_selected_match_triggered() {
  ...
  if (!currentIndex.isValid()) {
    QMessageBox::critical(
        this, "No match selected",
        "There is no match selected. Please select a match from the list.");
    return;
  }

  int round = ui->roundsList->currentRow();
  if (round >= m_model->rounds()) {
    QMessageBox::critical(this, "Invalid match",
                          "Invalid round selected for export.");
    return;
  }
  ...
  if (ongoing_round < round ||
      (ongoing_round == round && roundBoard.second <= board)) {
    QMessageBox::critical(this, "Error",
                          "This match hasn't happened yet. You can only export "
                          "it after the scores for this match have arrived.");
    return;
  }

  QString saveFilepath = QFileDialog::getSaveFileName(
      this, "Browse to a location for saving the match", QDir::homePath(),
      "XML document (*.xml)");
  if (saveFilepath.isEmpty())
    return;
...
void MainWindow::loadLogFile(QString filepath) {
  stopLogger();
  m_logger = new Logger(this);
  connect(m_logger, SIGNAL(newEvent(int, int, int, int)), this,
          SLOT(initialiseView(int, int, int, int)));
  if (m_logger->loadEvent(filepath, m_model))
    ...
    QMessageBox::critical(
        this, "Error",
        "There was an error loading the file. It is either not readable or a "
        "wrong file is selected. Please try again.");
  }
}

void MainWindow::on_actionPlay_a_match_triggered() {
  QString logFile =
      QFileDialog::getOpenFileName(this, "Browse to the match file",
                                   QDir::homePath(), "XML document (*.xml)");
  if (logFile.isEmpty())
    return;
  ...
  connect(m_logger, SIGNAL(setBoardForMatch(QDomElement)), m_player,
          SLOT(setBoard(QDomElement)));
  connect(m_logger, SIGNAL(setMovesForMatch(QDomNodeList)), m_player,
          SLOT(setMoves(QDomNodeList)));
...
void MainWindow::on_actionP_lay_selected_match_triggered() {
  QModelIndex currentIndex = ui->tablesList->currentIndex();
  if (!currentIndex.isValid()) {
    QMessageBox::critical(this, "No match selected",
                          "There is no match selected to export. Please select "
...
void MainWindow::on_actionChat_triggered() { m_chat->show(); }

void MainWindow::on_actionVideo_Chat_triggered() {
  this->m_video_chat_Manager->showWindow();
}
