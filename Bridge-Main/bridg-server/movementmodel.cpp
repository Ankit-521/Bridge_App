...
QList<QStandardRow> MovementModel::prepareRows(QList<int> pairs)
{
    ...
    foreach(int p, pairs) {
        ...
        for (int i=0; i<m_sheets.size(); i++) {
            int points = m_sheets[i].points(p);/// p is pair no no i is Board no
            result_board[p]+=(","+QString::number(points));
            total += points;
    ...
    for (i = result_board.begin(); i != result_board.end(); ++i)
